...
df_updated = pd.read_csv('DMA_MPEG_DASH_CSV.csv')
...
throughput_values.plot(label="Durchsatz (Bytes/s)", color='blue')
...
plt.tight_layout()
plt.plot()
plt.savefig(os.path.join(directory_name, 'Durchsatz.png'))
...
plt.title("Paketverlust (TCP-Neuübertragungen) über die Zeit")
plt.xlabel("Zeit (s)")
...
plt.tight_layout()
plt.plot()
plt.savefig(os.path.join(directory_name, 'PackageLoss.png'))
...
plt.figure(figsize=(14, 5))
average_jitter_values.plot(label="Durchschnittlicher Jitter (s)", color='green')
...
plt.savefig(os.path.join(directory_name, 'Jitter.png'))
